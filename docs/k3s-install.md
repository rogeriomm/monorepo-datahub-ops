...
```shell
kubectl apply -f ./k3s/custom-resources-pvel.yaml
```
```shell
kubectl apply -f ./k3s/custom-resources-pve.yaml
```
```shell
kubectl apply -f ./k3s/custom-resources-world1l.yaml
```
...
Install MetalLb
- https://serverfault.com/questions/1166268/why-cant-i-put-metallb-on-k3s
...
helm install --create-namespace --namespace metallb-system metallb metallb/metallb \
      --values ./k8s/metallb/$CLUSTER/helm/values.yaml 
```

```shell
kubectl apply -f ./k8s/metallb/$CLUSTER/kustomize/address-pool.yaml
````
...
argo-cd	argo-cd  	4       	2025-07-11 14:00:23.120809253 +0000 UTC	deployed	argo-cd-9.0.29	3.0.11     
```
